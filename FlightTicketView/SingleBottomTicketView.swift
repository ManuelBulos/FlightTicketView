...
open class SingleBottomTicketView: UITableViewCell {
    ...
    override open func draw(_ rect: CGRect) {
        ...
        backgroundColor = appearance.cellsBackgroundColor != nil ? appearance.cellsBackgroundColor:appearance.singleBottomCellBackgroundColor
        drawBottomTicket()
    ...
    func setElementsPostions() {
        ...
        let difference: CGFloat = UIDevice.current.orientation == .portrait ? 1.8:3.2
        let topTextFontSize: CGFloat = frame.width / (frame.height / (elementsMargin - difference))
        let BottomTextFontSize: CGFloat = frame.width / (frame.height / (elementsMargin - (difference + 1)))
    ...
    func createBezierPath() -> UIBezierPath {
        ...
        path.addLine(to: CGPoint(x: startingXPoint, y: circleRadius * 2))
